...
lib.back = {
    ...
    general: function() {
		if (VK_BLUE) {
			alert('vk blue');
			if (document.title == "SMARTGAMES.PORTAL") {
				window.close();
			} else {
				window.location = "http://smartgames.tv/GameTest/fPortal/index.php?brand=GameTest";
			}
		}
        else if (VK_BACK || VK_BACK_SPACE || VK_BACK_OPERA) {
			alert('vk back');
            if (document.title == "SMARTGAMES.PORTAL") {
                window.close();
            } else {
